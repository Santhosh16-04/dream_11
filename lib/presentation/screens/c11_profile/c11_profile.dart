...
class _M11_ProfileScreenState extends State<M11_ProfileScreen> {
  ...
  Widget _buildVerificationRow(
      String label, IconData icon, String status, VoidCallback onTap) {
    Color iconColor = status == 'verified'
        ? Colors.green
        : (status == 'under_process' ? Colors.orange : Colors.grey);

    // Get the actual value based on label
    String displayText = _getDisplayText(label);
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 2),
                  Text(
                    hasValue ? displayText : "Please verify your $label",
                    style: TextStyle(
                      fontSize: 14,
                      fontWeight: FontWeight.w500,
  ...
  String _getDisplayText(String label) {
    switch (label.toLowerCase()) {
      ...
      case 'aadhaar':
        if (verification['aadhaar_status'] == 'verified' ||
            verification['aadhaar_status'] == 'under_process') {
          return verification['aadhaar_name'] ?? '';
        }
        return '';
      case 'pan':
        if (verification['pan_status'] == 'verified' ||
            verification['pan_status'] == 'under_process') {
          return verification['pan_name'] ?? '';
        }
        return '';
      case 'bank':
        if (verification['bank_status'] == 'verified' ||
            verification['bank_status'] == 'under_process') {
          return verification['bank_name'] ?? '';
        }
        return '';
      case 'address':
        if (verification['address_verified'] == true) {
